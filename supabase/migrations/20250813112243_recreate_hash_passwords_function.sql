-- Eliminar función anterior y crear una nueva
DROP FUNCTION IF EXISTS hash_plain_passwords();

-- Función para hashear contraseñas existentes
CREATE OR REPLACE FUNCTION hash_plain_passwords()
RETURNS TABLE(user_email text, status text) AS $$
DECLARE
    user_record RECORD;
...
BEGIN
    ...
    FOR user_record IN 
        SELECT id, usuarios.email, password_hash 
        FROM usuarios 
