-- Corregir función para manejar productos sin UUID previo
BEGIN;
...
BEGIN
    ...
    IF NOT FOUND OR v_producto_carga_uuid IS NULL THEN
        -- Crear un registro temporal con los datos proporcionados
        v_producto_carga := ROW(
            NULL, -- id
            NULL, -- carga_id
            COALESCE(p_producto_nombre, 'Producto sin nombre'),
            'Insumo'::tipo_producto, -- tipo_producto
            COALESCE(p_producto_marca, 'Sin marca'),
            COALESCE(p_producto_modelo, 'Sin modelo'),
            NULL, -- numero_serie
            p_cantidad_cajas,
            NULL, -- observaciones
            NULL, -- imagen
...
-- Actualizar permisos
GRANT EXECUTE ON FUNCTION public.procesar_ingreso_fraccionado(TEXT, INTEGER, INTEGER, BOOLEAN, TEXT, TEXT, TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.procesar_ingreso_fraccionado(TEXT, INTEGER, INTEGER, BOOLEAN, TEXT, TEXT, TEXT, TEXT) TO anon;
