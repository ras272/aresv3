...
-- Crear enums para el sistema de repuestos (manejando posibles existentes)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_repuesto') THEN
    CREATE TYPE estado_repuesto AS ENUM ('Disponible', 'Reservado', 'En_uso', 'Dañado', 'Vencido');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_movimiento_repuesto') THEN
...
DROP INDEX IF EXISTS idx_repuestos_codigo;
DROP INDEX IF EXISTS idx_repuestos_nombre;
DROP INDEX IF EXISTS idx_repuestos_estado;
DROP INDEX IF EXISTS idx_repuestos_categoria;
DROP INDEX IF EXISTS idx_repuestos_fecha_vencimiento;
DROP INDEX IF EXISTS idx_repuestos_tags;
DROP INDEX IF EXISTS idx_repuestos_custom_fields;
DROP INDEX IF EXISTS idx_repuestos_activo;
DROP INDEX IF EXISTS idx_movimientos_repuesto;
...
DROP INDEX IF EXISTS idx_repuestos_equipos_repuesto;
DROP INDEX IF EXISTS idx_repuestos_equipos_equipo;
DROP INDEX IF EXISTS idx_repuestos_equipos_mantenimiento;
DROP INDEX IF EXISTS idx_repuestos_equipos_fecha;

-- Crear índices para repuestos_stock
...
-- Crear índices para movimientos_repuestos
...
-- Crear índices para repuestos_equipos
...
-- Eliminar triggers existentes si hay conflicto
DROP TRIGGER IF EXISTS update_repuestos_stock_updated_at ON repuestos_stock;
DROP TRIGGER IF EXISTS trigger_registrar_movimiento_repuesto ON repuestos_stock;
...
-- Eliminar vistas existentes si hay conflicto
DROP VIEW IF EXISTS vista_repuestos_por_equipo;
DROP VIEW IF EXISTS vista_movimientos_repuestos_recientes;

-- Vista de uso de repuestos por equipo
CREATE OR REPLACE VIEW vista_repuestos_por_equipo AS
SELECT 
    re.*,
...
CREATE OR REPLACE VIEW vista_movimientos_repuestos_recientes AS
SELECT 
    mr.*,
    rs.nombre as repuesto_nombre,
    rs.codigo_repuesto
...
-- Insertar algunos repuestos de ejemplo (solo si no existen)
INSERT INTO repuestos_stock (codigo_repuesto, nombre, descripcion, marca, modelo, cantidad_actual, cantidad_minima, estado, categoria, proveedor) 
SELECT generar_codigo_repuesto('REP'), 'Cable de Alimentación', 'Cable de alimentación estándar para equipos médicos', 'Genérico', 'CA-220V', 20, 5, 'Disponible', 'Cables', 'Proveedor General'
WHERE NOT EXISTS (SELECT 1 FROM repuestos_stock WHERE nombre = 'Cable de Alimentación');

INSERT INTO repuestos_stock (codigo_repuesto, nombre, descripcion, marca, modelo, cantidad_actual, cantidad_minima, estado, categoria, proveedor) 
SELECT generar_codigo_repuesto('REP'), 'Fusible 5A', 'Fusible de vidrio 5A para protección de equipos', 'Genérico', 'F-5A', 100, 20, 'Disponible', 'Fusibles', 'Proveedor General'
WHERE NOT EXISTS (SELECT 1 FROM repuestos_stock WHERE nombre = 'Fusible 5A');

INSERT INTO repuestos_stock (codigo_repuesto, nombre, descripcion, marca, modelo, cantidad_actual, cantidad_minima, estado, categoria, proveedor) 
SELECT generar_codigo_repuesto('REP'), 'Sensor de Temperatura', 'Sensor NTC de temperatura para monitoreo de equipos', 'Texas Instruments', 'TMP36', 15, 5, 'Disponible', 'Sensores', 'DigiKey'
WHERE NOT EXISTS (SELECT 1 FROM repuestos_stock WHERE nombre = 'Sensor de Temperatura');
