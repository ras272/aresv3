...
function ProductoAgrupadoItem({ grupo, tieneNumerosSerie, stockTotal, onAgregarProducto }: ProductoAgrupadoItemProps) {
  ...
  return (
    <div className="border rounded-lg p-4 space-y-3">
      ...
      {/* 📦 SELECTOR DE FRACCIONAMIENTO REDISEÑADO */}
        ...
        <div className="border border-gray-200 rounded-lg p-4 mt-3 bg-gray-50">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-gray-900">Configurar Venta</span>
            </div>
            <Button
              variant="ghost"
              size="sm"
          ...
          {/* Información de stock compacta */}
          {itemFraccionamiento && (
            <div className="bg-white border rounded p-3 mb-4">
              <div className="text-xs text-gray-500 mb-2">
                Stock disponible ({itemFraccionamiento.unidades_por_paquete} unidades/caja)
              </div>
              <div className="flex justify-between text-sm">
                <span>📦 {itemFraccionamiento.cajas_completas} cajas</span>
                <span>🔗 {itemFraccionamiento.unidades_sueltas} sueltas</span>
                <span className="font-medium">Total: {unidadesDisponiblesTotales}</span>
              </div>
            </div>
          )}

          {/* Selector tipo radio buttons */}
          <div className="space-y-3 mb-4">
            <div className="text-sm font-medium text-gray-700">Tipo de venta:</div>
            ...
            <label className={`flex items-center p-3 border rounded-lg cursor-pointer ${
              ...
              <input
                ...
                name="tipoVenta"
                value="unidad"
                checked={tipoVentaSeleccionado === 'unidad'}
                onChange={() => {
                ...
                disabled={unidadesDisponiblesTotales === 0}
                className="sr-only"
              />
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-sm">🔗 Por unidad</div>
                    <div className="text-xs text-gray-500">Vender unidades sueltas</div>
                  </div>
                </div>
            ...
            <label className={`flex items-center p-3 border rounded-lg cursor-pointer ${
              ...
              <input
                ...
                name="tipoVenta"
                value="caja"
                checked={tipoVentaSeleccionado === 'caja'}
                onChange={() => {
                  if (maxCajas > 0) {
              ...
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-sm">📦 Por caja</div>
                    <div className="text-xs text-gray-500">{unidadesPorCaja} unidades/caja</div>
                  </div>
                </div>
                {maxCajas === 0 && (
                  <span className="text-xs text-red-500">Sin cajas</span>
                )}
              </div>
            </label>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Input
                type="number"
                value={tipoVentaSeleccionado === 'caja' ? cantidadCajas : cantidadUnidades}
                onChange={(e) => {
                  const value = parseInt(e.target.value) || 1;
                  if (tipoVentaSeleccionado === 'caja') {
                    setCantidadCajas(Math.max(1, Math.min(maxCajas, value)));
                  } else {
                    setCantidadUnidades(Math.max(1, Math.min(unidadesDisponiblesTotales, value)));
                  }
                }}
                className="w-20 text-center h-9"
                min={1}
                max={tipoVentaSeleccionado === 'caja' ? maxCajas : unidadesDisponiblesTotales}
              />
              ...
              <Button
                ...
                disabled={
                  tipoVentaSeleccionado === 'caja' 
                    ? cantidadCajas >= maxCajas
                    : cantidadUnidades >= unidadesDisponiblesTotales
                }
                className="h-9 w-9 p-0"
              >
              ...
              <div className="ml-3 text-sm text-gray-600">
                = <span className="font-medium">{unidadesTotales}</span> unidades totales
              </div>
            </div>
          </div>
