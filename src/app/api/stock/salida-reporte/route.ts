...
import { registrarSalidaStockReporte } from "@/lib/database/stock";
...
export async function POST(request: NextRequest) {
  try {
    ...
    await registrarSalidaStockReporte(body);
    return NextResponse.json({ success: true }, { status: 201 });
  } catch (error: any) {
    console.error("/api/stock/salida-reporte POST error:", error);
